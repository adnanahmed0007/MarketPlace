  ...
  async function handleSubmit(e) {
    ...
    try {
      const response = await axios.post(
        'http://localhost:9808/api/sell/farmer/data/cropname/pickuplocation',
      ...
      if (response && response.status === 200) {
        ...
        setArrayget(response.data.findalllocaandcrop);
      }
    } catch (e) {
      if (e.response && e.response.status === 400) {
        alert('We can not find the crops');
      }
    }
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-emerald-700 to-teal-600 bg-clip-text text-transparent mb-4">
            Search Crops by Name & Location
          </h1>
          <p className="text-xl text-gray-600">
            Find available crops near your location
          </p>
        </div>
...
export default BuyLocationCropname;
